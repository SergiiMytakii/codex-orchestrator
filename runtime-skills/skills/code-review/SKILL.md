---
name: "code-review"
description: "Review code changes or existing code with an evidence-first two-track process. Use for code review, PR review, commit audit, regression scan, bug hunt, or review-and-fix work. Automatically fix only critical or high-severity high-confidence issues that satisfy the auto-fix policy. For spec-driven work, use the bounded Implementation Review Loop and reserve cleanup for its final gate by default."
---
...
The default shape is a two-reviewer process:
...
The main agent is the coordinator. It pins the review target, runs both tracks when possible, verifies their strongest findings, applies only safe fixes, and returns a concise findings-first report.
...
For medium or large implementation changes, run ``cleanup-review`` first, integrate safe cleanup fixes, rerun relevant checks, then run ``code-review`` on the settled diff.

Exception for approved spec execution: follow
`../../shared/docs/agents/implementation-review-loop.md`. Intermediate code-review
checkpoints do not automatically run cleanup first; final cleanup and final code
review use the shared whole-spec budget.
...
Do not infer a fresh allowance, choose another mode, or make the Module's
...
- Contract test ledger: `../../shared/docs/agents/contract-test-ledger.md`
- Shared confidence rubric: `../../shared/docs/agents/confidence-rubric.md`

Load `references/framework-lenses.md` when the user names a framework or files/configs strongly imply one. Load `references/targeted-recipes.md` and `../../shared/docs/agents/contract-test-ledger.md` when the diff shape matches new fields, retries, DTO/schema/runtime contracts, caches, state merge precedence, ordering, evidence/snapshots, determinism, or aggregation summaries. Load `references/bug-classes.md` for substantial reviews or broad bug hunts.
...
### 4. Run Two Review Tracks

Run two `reviewer_deep` agents in parallel when Runner-route is authorized and the role is available; invoking ``code-review`` authorizes these review tracks. Preserve every fulfilled launch handle even if its peer fails, then close all launched children. Give each one a distinct brief and tell it not to edit or revert unrelated work.
...
When already inside `reviewer_deep`, keep both tracks separate. If the user explicitly forbids Runner-route, run both tracks inline and report the missing independence; otherwise use inline fallback only when the shared routing policy permits it.
...
4. Reclassify severity/confidence using `../../shared/docs/agents/confidence-rubric.md` if evidence does not support the label.
...
- confidence is high under `../../shared/docs/agents/confidence-rubric.md`
- root cause is clear
