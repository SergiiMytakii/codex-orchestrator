...
test('workflow source v2 binds dependency skills and keeps evals outside runtime operation closure', async () => {
  ...
  for (const skill of ['alpha', 'beta']) {
    ...
    await writeFile(join(codexHome, 'skills', skill, 'SKILL.md'), skill === 'beta'
      ? '# beta\n\nUse [dependency policy](../../docs/agents/dependency.md).\n'
      : '# alpha\n');
  ...
  const missingDependencyReference = structuredClone(manifest);
  missingDependencyReference.skills.beta.files = missingDependencyReference.skills.beta.files
    .filter((path: string) => path !== 'docs/agents/dependency.md');
  missingDependencyReference.operations.alpha.files = missingDependencyReference.operations.alpha.files
    .filter((path: string) => path !== 'docs/agents/dependency.md');
  rehashV2Manifest(missingDependencyReference);
  await writeFile(join(outputRoot, 'manifest.json'), `${canonicalJson(missingDependencyReference)}\n`);
