...
- First determine provenance. If the issue was generated by `$to-tickets`, do
  not draft a spec: its body and inherited parent Contract IDs are already the
  execution authority. Return a ticket-flow defect if they are incomplete.
- For a standalone raw issue, treat its acceptance criteria as source authority
  and use parent material only as explicitly inherited context.
- Read comments for changed decisions, blockers, credentials, external contracts, live prerequisites, and rejected approaches.
- Preserve relevant external contracts, verification, and blocker content.
