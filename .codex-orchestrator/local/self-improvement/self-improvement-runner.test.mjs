...
test('implement command builds then runs targeted issue and never calls daemon', async () => {
  ...
  try {
    ...
    assert.deepEqual(exec.calls.map((call) => [call.command, call.args]), [
      ...
      ['node', ['dist/src/v2/cli.js', 'run', '--target', runner.paths.cwd, '--issue', '123']],
    ]);
...
test('daily implements one code issue and never runs review backlog follow-up publication', async () => {
  const exec = makeExecStub({
    ...
    [commandKey('node', ['dist/src/v2/cli.js', 'run', '--target', '<cwd>', '--issue', '222'])]: {
  ...
  const failingExec = makeExecStub({
    ...
    [commandKey('npm', ['run', 'build', '--silent'])]: { code: 0, stdout: '' },
    [commandKey('node', ['dist/src/v2/cli.js', 'run', '--target', '<cwd>', '--issue', '223'])]: {
...
test('daily reuses an open auto self-improvement issue instead of discovering another one', async () => {
  const exec = makeExecStub({
    ...
    [commandKey('npm', ['run', 'build', '--silent'])]: { code: 0, stdout: '' },
    [commandKey('node', ['dist/src/v2/cli.js', 'run', '--target', '<cwd>', '--issue', '310'])]: {
