...
export interface SpecDeliveryV1 {
  ...
  runId: string;
  workflowGenerationSha256: string;
  stage: 'authoring' | 'answer-authoring' | 'question' | 'review' | 'author-repair' | 'approved' | 'frozen' | 'rejected' | 'exhausted';
...
export function createInitialSpecDelivery(input: {
  ...
  return {
    ...
    review: { reviewer: null, coverage: [], defects: [], acceptedRisks: [], acceptedReportSha256: null },
...
export function acceptSpecRevision(state: SpecDeliveryV1, revision: SpecRevisionV1): SpecDeliveryV1 {
  ...
  const defects = state.stage === 'author-repair'
    ? state.review.defects.map((defect) => ['open','reopened'].includes(defect.status)
  ...
  const next: SpecDeliveryV1 = {
    ...structuredClone(state), stage: 'review',
    revisions: [...state.revisions, validated], authorSessionId: revision.author.sessionId,
    review: { ...structuredClone(state.review), reviewer: null, defects, acceptedReportSha256: null },
  };
...
export function acceptSpecReview(state: SpecDeliveryV1, report: SpecReviewReportV1, reportSha256: string): SpecDeliveryV1 {
  ...
  if (state.stage !== 'review') throw new Error('spec is not ready for a review result');
  ...
  if (report.targetRevision !== target.revision || report.targetSha256 !== target.revisionSha256
    ...
    || (state.review.reviewer !== null && report.reviewer.sessionId !== state.review.reviewer.sessionId)) throw new Error('spec review correlation mismatch');
  ...
  const defects = mergeFullDefects(state, report);
  ...
  const next: SpecDeliveryV1 = {
    ...
    review: {
      reviewer: structuredClone(report.reviewer), coverage: sorted(report.coverage),
      defects: defects.sort((a,b) => a.id.localeCompare(b.id)),
      acceptedRisks: structuredClone(report.acceptedRisks), acceptedReportSha256: reportSha256,
    },
    budgets: {
...
export function validateSpecDelivery(value: unknown): SpecDeliveryV1 {
  ...
  if (!['authoring','answer-authoring','question','review','author-repair','approved','frozen','rejected','exhausted'].includes(value.stage as string)) throw new Error('spec delivery stage is invalid');
...
function mergeFullDefects(state: SpecDeliveryV1, report: SpecReviewReportV1): CodeReviewDefectV1[] {
  if (state.review.defects.length === 0) return structuredClone(report.defects);
  ...
  const merged = state.review.defects.map((defect) => {
    ...
    if (!next) throw new Error('full spec review omitted a previous defect ID');
    for (const field of ['id','class','invariant','failure','introducedTargetRevision'] as const) {
      if (canonicalJson(defect[field]) !== canonicalJson(next[field])) throw new Error(`full spec review changed immutable defect field: ${field}`);
    }
    reported.delete(defect.id);
    return structuredClone(next);
  });
  for (const defect of reported.values()) {
    if (defect.status !== 'open' && defect.status !== 'reopened') throw new Error('full spec review introduced invalid defect');
...
function validateReviewReport(value: SpecReviewReportV1): void {
  exact(value, ['version','targetRevision','targetSha256','verdict','reviewer','coverage','defects','acceptedRisks'], 'spec review report');
  ...
  if (!['approved','needs-work','rejected'].includes(value.verdict)) throw new Error('spec review verdict is invalid');
  strings(value.coverage, 'review coverage');
  if (!Array.isArray(value.defects) || !Array.isArray(value.acceptedRisks)) throw new Error('spec review collections are invalid');
...
function validateReviewState(value: unknown): asserts value is SpecDeliveryV1['review'] {
  exact(value, ['reviewer','coverage','defects','acceptedRisks','acceptedReportSha256'], 'spec review state');
  if (value.reviewer !== null) actor(value.reviewer, 'persisted reviewer');
  strings(value.coverage,'persisted coverage');
